...
class Player(GameSprite):
    ...
    def fire(self):
        if self.reload >= 20:
            bullet = Bullet('fire2.png', self.rect.x, self.rect.y,
                        5, 15, 30)
            bullets.add(bullet)
            self.reload = 0
            fire_snd.play()
    ...
    def draw(self):
        ...
        rect1 = Rect(self.rect.x, self.rect.bottom, 
                        self.rect.w*self.health/100, 5)
        
        rect2 = Rect(self.rect.x, self.rect.bottom, 
                        self.rect.w, 5)
        color = (
                255 - int(255*self.health/100),
...
background = transform.scale(image.load("fon1.jpg"), (win_width, win_height))
picture_win = transform.scale(image.load("win2.jpg"), (win_width, win_height))
picture_loos = transform.scale(image.load("gameover.jpg"), (win_width, win_height))
...
boom_sprites = sprites_load('boom', 'boom', (70, 70), (0,0,0))
...
while game:
    ...
    if not finish:        
        ...
        if ticks % 60 == 0: 
            size = randint(70, 80)           
            ufo = Ufo('ufo1.png', randint(0, win_width), -100,
        ...
        if win:
            window.blit(picture_win, (0,0))
        else:
            # если конец игры
            window.blit(picture_loos, (0,0))
